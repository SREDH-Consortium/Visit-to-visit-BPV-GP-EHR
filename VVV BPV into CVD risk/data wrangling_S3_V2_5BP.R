...
patients_bp_outcome_CVD_55_S3_more5 <- bp_outcome_CVD_55_S3 %>%
  add_count(Patient_UUID) %>%
  filter(n >= 5)
distinct.patients_bp_outcome_CVD_55_S3_more5 <- patients_bp_outcome_CVD_55_S3_more5 %>%
  distinct(Patient_UUID)

patients_bp_outcome_CVD_55_S3_more5 <- patients_bp_outcome_CVD_55_S3_more5 %>%
  left_join(
    cleaned_outcome_55 %>%
...
# Calculating the number of outcome
patients_bp_outcome_CVD_55_S3_more5 %>%
  distinct(Patient_UUID, outcome) %>%
  mutate(outcome_group = case_when(
    is.na(outcome) ~ "NA",
    outcome == 1 ~ "1",
...
eligible_analysis_CVD_55_S3_5 <- patients_bp_outcome_CVD_55_S3_more5 %>%
  select (-n)
...
eligible_analysis_CVD_55_S3_5 <- eligible_analysis_CVD_55_S3_5 %>%
...
eligible_patients_55_5 <- patient_age_counts_55 %>%
...
filtered_bp_sd_5_for55 <- bp_sd %>%
  filter(Patient_UUID %in% eligible_patients_55_5)

distinct.filtered_bp_sd_5_for55 <- filtered_bp_sd_5_for55 %>%
  distinct(Patient_UUID)


intersect_5bp_outcome_55_CVD <- inner_join(
  cleaned_outcome_55,
  distinct.filtered_bp_sd_5_for55,
  by = "Patient_UUID"
)

distinct.intersect_5bp_outcome_55_CVD <- intersect_5bp_outcome_55_CVD %>%
  distinct(Patient_UUID)

# intersect_bp_outcome_55_CVD <- inner_join(
#   intersect_5bp_outcome_55_CVD,
#   distinct.filtered_bp_sd_5_for55,
#   by = "Patient_UUID"
# )
...
all_ids_55 <- union(
  intersect_5bp_outcome_55_CVD$Patient_UUID,
  distinct.filtered_bp_sd_5_for55$Patient_UUID
)

# UUIDs that were in the inner join
intersect_ids_55 <- intersect_5bp_outcome_55_CVD$Patient_UUID
...
########checking if patient in <55 and >=55 is listed in the patients_bp_outcome_CVD_55_S3_more5
...
eligible_analysis_55_add_5 <- bp_anti_not_in_intersect_55 %>%
  mutate(outcome = 0)

eligible_analysis_55_add_final_5 <- eligible_analysis_55_add_5 %>%
...
eligible_analysis_55_add_final_5 <- eligible_analysis_55_add_final_5 %>% 
...
# Now bind safely
eligible_analysis_CVD_55_S3_final_5 <- rbind(eligible_analysis_CVD_55_S3_5, eligible_analysis_55_add_final_5)

common_ids <- intersect(
  eligible_analysis_CVD_55_S3_5$Patient_UUID,
  eligible_analysis_55_add_final_5$Patient_UUID
)

length(common_ids)  # To see how many are common

distinct.eligible_analysis_CVD_55_S3_final_5 <- eligible_analysis_CVD_55_S3_final_5 %>%
  distinct (Patient_UUID)

# Optional: View the overlapping Patient_UUIDs
# common_ids_df <- data.frame(Patient_UUID = common_ids)

write.csv(eligible_analysis_CVD_55_S3_final_5, "eligible_analysis_CVD_55_S3_5_BP_final.csv", row.names = FALSE)

# Calculating the number of outcome
eligible_analysis_CVD_55_S3_final_5 %>%
  distinct(Patient_UUID, outcome) %>%
  mutate(outcome_group = case_when(
    is.na(outcome) ~ "NA",
    outcome == 1 ~ "1",
...
na_outcome_ids <- eligible_analysis_CVD_55_S3_final_5 %>%
...
summary_S <- eligible_analysis_CVD_55_S3_final_5 %>%
  group_by(Patient_UUID) %>%
  summarise(
...
summary_D <- eligible_analysis_CVD_55_S3_final_5 %>%
  group_by(Patient_UUID) %>%
  summarise(
...
# 3. ARV for Systolic
arv_S <- eligible_analysis_CVD_55_S3_final_5 %>%
...
# 4. ARV for Diastolic
arv_D <- eligible_analysis_CVD_55_S3_final_5 %>%
  arrange(Patient_UUID, ObservationDate) %>%
  group_by(Patient_UUID) %>%
  mutate(
...
eligible_analysis_CVD_55_S3_final_5$outcome_date <- as.Date(eligible_analysis_CVD_55_S3_final_5$outcome_date, format = "%Y-%m-%d")


unique_outcome_date <- eligible_analysis_CVD_55_S3_final_5 %>%
  select(Patient_UUID, outcome_date, outcome) %>%
  distinct(Patient_UUID, .keep_all = TRUE)

# Perform the join
BPV_55_CVD_S3_5BP <- bp_summary %>%
  left_join(unique_dob_gender, by = "Patient_UUID")

BPV_55_CVD_S3_5BP <- BPV_55_CVD_S3_5BP %>%
...
BPV_55_neg_ms <- BPV_55_CVD_S3_5BP %>%
  filter(time_ms < 0)

BPV_55_CVD_S3_5BP_final <- BPV_55_CVD_S3_5BP %>%
  filter(time_ms > 0)
...
write.csv(BPV_55_CVD_S3_5BP_final, "BPV_55_CVD_S3_5BP_final_V1.csv", row.names = FALSE)


BPV_55_CVD_S3_5BP_final <- BPV_55_CVD_S3_5BP_final %>%
  mutate(
    outcome_date = as.Date(outcome_date, format = "%Y-%m-%d"),  # adjust if format is different
    DateOfBirth = as.Date(DateOfBirth, format = "%Y-%m-%d"),    # same here
    outcome_age = ceiling(as.numeric(difftime(outcome_date, DateOfBirth, units = "days")) / 365.25)
...
BPV_55_CVD_S3_5BP_final %>%
  summarise(
    min_age  = min(outcome_age, na.rm = TRUE),
    max_age  = max(outcome_age, na.rm = TRUE),
    mean_age = mean(outcome_age, na.rm = TRUE),
    sd_age   = sd(outcome_age, na.rm = TRUE)
  )


tc_hdl <- lab_tc_hdl_v2_JJ

tc_hdl <- tc_hdl %>%
  mutate(
    TC_Value_JJ = ifelse(is.na(TC_Value_JJ), Value * HDL_Value_JJ, TC_Value_JJ)
  )

tc_hdl_only <- tc_hdl %>%
  select (Patient_UUID, TC_Value_JJ, HDL_Value_JJ)

BPV_55_CVD_5BP <- BPV_55_CVD_5BP %>%
  left_join(tc_hdl_only, by = "Patient_UUID")

BPV_55_CVD_5BP <- BPV_55_CVD_5BP %>%
  rename (TC_Value = TC_Value_JJ, HDL_Value = HDL_Value_JJ)

write.csv(BPV_55_CVD_5BP, "BPV_55_CVD_5BP_final.csv", row.names = FALSE)
