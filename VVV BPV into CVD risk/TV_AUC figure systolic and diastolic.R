...
bp_data <- data.frame(
  ...
  SDBP_AUC = c(0.700, 0.641, 0.690, 0.717, 0.752),
  SDBP_lower = c(0.509, 0.484, 0.597, 0.655, 0.672),
  SDBP_upper = c(0.857, 0.798, 0.782, 0.778, 0.831),
  BPV_SD_AUC = c(0.806, 0.799, 0.841, 0.848, 0.867),
  BPV_SD_lower = c(0.722, 0.720, 0.798, 0.806, 0.825),
  BPV_SD_upper = c(0.890, 0.877, 0.883, 0.889, 0.909),
  BPV_CV_AUC = c(0.753, 0.759, 0.821, 0.842, 0.869),
  BPV_CV_lower = c(0.665, 0.673, 0.774, 0.802, 0.830),
  BPV_CV_upper = c(0.841, 0.846, 0.867, 0.880, 0.908)
)
...
bp_long <- bp_data %>%
  pivot_longer(
    ...
    names_pattern = "(SDBP|BPV_SD|BPV_CV)_(AUC|lower|upper)"
  ) %>%
  mutate(measure = recode(measure,
                          SDBP = "Single BP Measurement",
                          BPV_SD = "Visit-to-visit BPV (SD)",
                          BPV_CV = "Visit-to-visit BPV (CV)"),
         measure = factor(measure, levels = c("Single BP Measurement", "Visit-to-visit BPV (SD)", "Visit-to-visit BPV (CV)")))
...
ggsave("AUC_comparison_plot_SD.png", plot = auc_plot, width = 10, height = 6, dpi = 600)
