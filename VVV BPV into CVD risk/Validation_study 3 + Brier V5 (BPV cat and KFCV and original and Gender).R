...
for (g in gender_levels) {
  ...
  # Create folds within gender (based on first imputation)
  data_g1 <- complete(imputed_with_orig, 1) %>% filter(Gender == g)
  n_g <- nrow(data_g1)
  ...
  for (m_idx in 1:imputed_with_orig$m) {
    ...
    for (k in 1:K_folds) {
      ...
      fit <- coxph(
        Surv(time_ms, outcome) ~ SmokingStatus + fglucose + bmi +
          Value_TCHDL + egfr + FH + com_ht + com_dm + sd_S_cat,
        data = train_data,
        x = TRUE,
        y = TRUE
      )
