...
med <- Medications %>%
  filter(Patient_UUID %in% BPV_55_3022$Patient_UUID)
...
distinct_drugnames <- filtered_med_ht %>%
  distinct(DRUGNAME)

clean_drugnames <- distinct_drugnames %>%
  mutate(
    DRUGNAME_clean = DRUGNAME %>%
      str_to_lower() %>%                                    # make lowercase
      str_replace_all("\\b\\d+\\s*(mg|g|mcg|ml|mg/ml)\\b", "") %>%   # remove strength like 10 mg
      str_replace_all("\\b\\d+\\b", "") %>%                 # remove standalone numbers
      str_replace_all("\\b(tablet|tab|capsule|cap|caps|syrup|inj|injection|patch|cream|ointment)\\b", "") %>%
      str_replace_all("[[:punct:]]", " ") %>%                # remove punctuation
      str_squish()                                           # trim extra spaces
  )

distinct_drugnames_clean <- clean_drugnames %>%
  distinct(DRUGNAME_clean)

write.csv (distinct_drugnames_clean, "distinct_drugnames_clean_S2.csv", row.names = FALSE)
