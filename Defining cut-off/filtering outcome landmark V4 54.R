...
additional_outcome_1_56_formated <- additional_outcome_1_56...FORMATTED
...
outcome_1_54 <- outcome_1 %>%
  filter(outcome_age >= 54)


write.csv(outcome_1, "outcome_1_V2.csv", row.names = FALSE)
write.csv(outcome_1_54, "outcome_1_54.csv", row.names = FALSE)
...
no.CVD.gp.40 <- no.CVD.gp %>%
  filter(outcome_age >= 40)

# distinct patients_h_outcome_1835
distinct.no.CVD.gp.40 <- no.CVD.gp.40 %>%
  distinct(Patient_UUID)



# filtering the latest visit date
latest.VisitDate.no.CVD.gp.40 <- no.CVD.gp.40 %>%
  group_by(Patient_UUID) %>%
...
no.CVD.h.40 <- no.CVD.h %>%
  filter(outcome_age >= 40)


# obtaining the latest visit date
latest.VisitDate.no.CVD.h.40 <- no.CVD.h.40 %>%
  group_by(Patient_UUID) %>%
...
latest.VisitDate.no.CVD.gp.40 <- latest.VisitDate.no.CVD.gp.40 %>%
...
latest.VisitDate.no.CVD.h.40 <- latest.VisitDate.no.CVD.h.40 %>%
  select(-PERSON_ID)
latest.VisitDate.no.CVD.h.40 <- latest.VisitDate.no.CVD.h.40 %>%
  rename(VisitDate = ENCOUNTER_DATE)

# Combine the datasets for each landmark
combined_noCVD.40 <- rbind(latest.VisitDate.no.CVD.gp.40, latest.VisitDate.no.CVD.h.40)
...
# Filter the dataset
selected_combined_noCVD_40 <- combined_noCVD.40 %>%
  filter(Patient_UUID %in% selected_uuids)

selected_combined_noCVD_40 <- selected_combined_noCVD_40 %>%
...
empty_diag_combined_noCVD_54 <- combined_noCVD.54 %>%
  filter(is.na(DIAGNOSIS_DESCRIPTION) | DIAGNOSIS_DESCRIPTION == "",
         is.na(DIAGNOSIS_CODE) | DIAGNOSIS_CODE == "")


clean_combined_noCVD_54 <- combined_noCVD.54 %>%
  filter(!( (is.na(DIAGNOSIS_DESCRIPTION) | DIAGNOSIS_DESCRIPTION == "") &
              (is.na(DIAGNOSIS_CODE) | DIAGNOSIS_CODE == "") ))

final_clean_combined_noCVD_54 <- clean_combined_noCVD_54 %>%
...
write.csv(clean_combined_noCVD_54, "clean_combined_noCVD_54.csv", row.names = FALSE)
write.csv(final_clean_combined_noCVD_54, "final_clean_combined_noCVD_54.csv", row.names = FALSE)

outcome_0_54 <- final_clean_combined_noCVD_54
outcome_0_54 <- outcome_0_54 %>%
  rename (outcome_date = VisitDate)

outcome_0_54 <- outcome_0_54 %>%
  select(-DIAGNOSIS_DESCRIPTION, -DIAGNOSIS_CODE, -ENCOUNTER_TYPE)

outcome_0_54$outcome <- 0


common_outcome_54_df <- data.frame(Patient_UUID = intersect(outcome_1_54$Patient_UUID, outcome_0_54$Patient_UUID))



complete_outcome_54 <- rbind (outcome_1_54, outcome_0_54)

cleaned_outcome_54 <- complete_outcome_54 %>%
  group_by(Patient_UUID) %>%
...
all(cleaned_outcome_54$Patient_UUID %in% complete_outcome_54$Patient_UUID)
all(complete_outcome_54$Patient_UUID %in% cleaned_outcome_54$Patient_UUID)


#write.csv(cleaned_outcome_54, "cleaned_outcome_54.csv", row.names = FALSE)
