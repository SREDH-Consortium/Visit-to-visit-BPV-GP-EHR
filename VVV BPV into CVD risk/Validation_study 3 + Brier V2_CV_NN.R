...
imputed = readRDS("imputed_55_CVD_complete_BPVcat.rds")
...
cox_models <- lapply(imputed_list, function(df) {
  ...
  coxph(Surv(time_ms, outcome) ~ Gender + SmokingStatus + tr_fglucose + tr_bmi + Value_TCHDL +
          egfr + FH + com_ht + com_dm + sd_S_cat,
...
# --- Extract pooled tvb values 12,24,36,48,60 ---
...
saveRDS(results_cv_S, file = "results_cv_S.rds")
...
cox_models <- lapply(imputed_list, function(df) {
  train_data <- df %>% filter(dataset == "train")
  coxph(Surv(time_ms, outcome) ~ Gender + SmokingStatus + tr_fglucose + tr_bmi + Value_TCHDL +
          egfr + FH + cv_D, data = train_data, x = TRUE, y = TRUE)
})
...
saveRDS(results_cv_D, file = "results_cv_D.rds")
...
cox_models <- lapply(imputed_list, function(df) {
  train_data <- df %>% filter(dataset == "train")
  coxph(Surv(time_ms, outcome) ~ Gender + SmokingStatus + tr_fglucose + tr_bmi + Value_TCHDL +
          egfr + FH + cv_S + cv_D, data = train_data, x = TRUE, y = TRUE)
})
...
saveRDS(results_cv_SD, file = "results_cv_SD.rds")
