...
bp_sd_V2$ObservationDate <- as.Date(bp_sd_V2$ObservationDate , format = "%Y-%m-%d")
wide_bp_1$ObservationDate <- as.Date(wide_bp_1$ObservationDate , format = "%Y-%m-%d")

bp_sd_V2$DateOfBirth <- as.Date(bp_sd_V2$DateOfBirth , format = "%Y-%m-%d")
wide_bp_1$DateOfBirth <- as.Date(wide_bp_1$DateOfBirth , format = "%Y-%m-%d")

complete_bp_1 <- rbind(bp_sd_V2, wide_bp_1)

wide_bp_0$ObservationDate <- as.Date(wide_bp_0$ObservationDate , format = "%Y-%m-%d")
wide_bp_0$DateOfBirth <- as.Date(wide_bp_0$DateOfBirth , format = "%Y-%m-%d")

complete_bp_2 <- rbind(complete_bp_1, wide_bp_0)

wide_bp_1_add$ObservationDate <- as.Date(wide_bp_1_add$ObservationDate , format = "%Y-%m-%d")
wide_bp_1_add$DateOfBirth <- as.Date(wide_bp_1_add$DateOfBirth , format = "%Y-%m-%d")
...
# Check for negative AgeAtMeasurement
negative_age <- complete_bp_3 %>% filter(AgeAtMeasurement < 0)
...
write.csv (bp_sd, "bp_sd_CVD_S3.csv", row.names = FALSE)
write.csv (bp_sd, "bp_sd_complete_updated.csv", row.names = FALSE)

# Filtering Patients who have BP measurement less than 55 y.o
bp_sd_less55_for55 <- bp_sd %>%
  filter(AgeAtMeasurement < 55)
distinct.bp_sd_less55_for55 <- bp_sd_less55_for55 %>%
  distinct(Patient_UUID)

write.csv (bp_sd_less55_for55, "bp_sd_less55_CVD_V2", row.names = FALSE)
...
n_outcome_LA55_for55 <- length(intersect(bp_sd_less55_for55$Patient_UUID, cleaned_outcome_55$Patient_UUID))

n_outcome_LA55_for55

bp_sd_less55_unique_for55 <- bp_sd_less55_for55 %>%
  distinct(Patient_UUID, .keep_all = TRUE)

complete_outcome_55_unique <- cleaned_outcome_55 %>%
  distinct(Patient_UUID, .keep_all = TRUE)

intersect_df_LA55_for55 <- inner_join(
  bp_sd_less55_unique_for55,
...
BPV_55_med_com_RF_lab_timems <- BPV_55_med_com_RF_lab %>%
  mutate(
...
BPV_55_neg_ms <- BPV_55_med_com_RF_lab_timems %>%
  filter(time_ms < 0)
...
# BPV_55_med_com_RF_lab_timews <- BPV_55_med_com_RF_lab %>%
#   mutate(
#     date_turns_55 = as.Date(DateOfBirth) %m+% years(55),
#     time_ws = ceiling(interval(date_turns_55, outcome_date) / weeks(1))
#   )
#
# BPV_55_neg_weeks <- BPV_55_med_com_RF_lab_timews %>%
#   filter(time_ws < 0)

################################################################

BPV_55_med_com_RF_lab_timems_final <- BPV_55_med_com_RF_lab_timems %>%
  filter(time_ms >= 0)

write.csv(BPV_55_med_com_RF_lab_timems_final, "BPV 55_3022.csv", row.names = FALSE)
