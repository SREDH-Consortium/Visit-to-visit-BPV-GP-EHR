...
# Step 3: Get cumulative patient counts from 3 to 20
...
write.csv (cumulative_summary_icc, "cumulative_summary_icc.csv", row.names = FALSE)
...
write.csv (sd_table, "sd_table_icc.csv", row.names = FALSE)
...
cor_results <- map_dfr(3:11, function(n) {
  col_n <- paste0("sd_S_", n)

  rcorr_out <- rcorr(sd_table[[col_n]], sd_table$sd_S_12, type = "pearson")

  tibble(
    n = n,
    r = rcorr_out$r[1, 2],
    p_value = rcorr_out$P[1, 2]
  )
})



icc_results <- map_dfr(3:11, function(n) {
  col_n <- paste0("sd_S_", n)
  ...
  # ONLY use irr::icc(), avoid psych::ICC()
  icc_out <- icc(df_pair, model = "twoway", type = "consistency", unit = "single")

  tibble(n = n, ICC = icc_out$value)
})

print (icc_results)


results <- map_dfr(3:11, function(n) {
  col_n <- paste0("sd_S_", n)
...
print(dim(df_pair))
print(sum(!is.na(df_pair$sd_n)))
print(sum(!is.na(df_pair$sd_12)))



####################calculating the SD for each number of measurements#################

library(dplyr)
library(purrr)

# Step 1: Compute SDs for n = 3 to 12, filtering patients with at least n readings
sd_list <- map(3:12, function(n) {
  icc_analysis_5 %>%
    group_by(Patient_UUID) %>%
    filter(n() >= n) %>%        # Only keep patients with at least n readings
    slice_head(n = n) %>%
    summarise(sd_S = sd(ObservationValue_S), .groups = "drop") %>%
    rename(!!paste0("sd_S_", n) := sd_S)
})

# Step 2: Combine all into one dataset by Patient_UUID
sd_combined_5 <- reduce(sd_list, full_join, by = "Patient_UUID")
