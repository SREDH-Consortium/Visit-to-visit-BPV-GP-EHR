...
write.csv(outcome_1_56, "outcome_1_56.csv", row.names = FALSE)
...
# distinct patients_h_outcome_1835
distinct.no.CVD.gp.56 <- no.CVD.gp.56 %>%
  distinct(Patient_UUID)
...
latest.VisitDate.no.CVD.gp.56 <- latest.VisitDate.no.CVD.gp.56 %>%
...
latest.VisitDate.no.CVD.h.56 <- latest.VisitDate.no.CVD.h.56 %>%
  select(-PERSON_ID)
latest.VisitDate.no.CVD.h.56 <- latest.VisitDate.no.CVD.h.56 %>%
...
# Filter the dataset
selected_combined_noCVD_56 <- combined_noCVD.56 %>%
  filter(Patient_UUID %in% selected_uuids)

selected_combined_noCVD_56 <- selected_combined_noCVD_56 %>%
...
empty_diag_combined_noCVD_56 <- combined_noCVD.56 %>%
  filter(is.na(DIAGNOSIS_DESCRIPTION) | DIAGNOSIS_DESCRIPTION == "",
         is.na(DIAGNOSIS_CODE) | DIAGNOSIS_CODE == "")


clean_combined_noCVD_56 <- combined_noCVD.56 %>%
  filter(!( (is.na(DIAGNOSIS_DESCRIPTION) | DIAGNOSIS_DESCRIPTION == "") &
              (is.na(DIAGNOSIS_CODE) | DIAGNOSIS_CODE == "") ))

final_clean_combined_noCVD_56 <- clean_combined_noCVD_56 %>%
...
write.csv(clean_combined_noCVD_56, "clean_combined_noCVD_56.csv", row.names = FALSE)
write.csv(final_clean_combined_noCVD_56, "final_clean_combined_noCVD_56.csv", row.names = FALSE)

outcome_0_56 <- final_clean_combined_noCVD_56
...
outcome_0_56$outcome <- 0


common_outcome_56_df <- data.frame(Patient_UUID = intersect(outcome_1_56$Patient_UUID, outcome_0_56$Patient_UUID))



complete_outcome_56 <- rbind (outcome_1_56, outcome_0_56)

cleaned_outcome_56 <- complete_outcome_56 %>%
  group_by(Patient_UUID) %>%
